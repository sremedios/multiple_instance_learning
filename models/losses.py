...
def continuous_dice_coef_loss(x, y, smooth=1):

    y_pred_f = tf.reshape(x, [-1])
    ...
    for i, (y_t, y_p) in enumerate(zip(y_true_f, y_pred_f)):
        sample_intersection = tf.reduce_sum(
            tf.reshape(y_t, [-1]) * tf.reshape(y_p, [-1]))
        if tf.cond(tf.greater(sample_intersection, 0.), lambda: 1, lambda: 0) == 1:
            c[i] = tf.reduce_sum(y_t * y_p) /\
                tf.reduce_sum(y_t * tf.sign(y_p))

    continuous_union = c * \
        tf.reduce_sum(y_true_f, axis=-1, keepdims=True) + \
        tf.reduce_sum(y_pred_f, axis=-1, keepdims=True)
