...
def joint_unet(shared_weights,
    ...
    seg_inputs = Input((None, None, num_channels), name='seg_inputs')
    ...
    seg_outputs = Conv2D(1, 1, activation='sigmoid', name='seg_outputs')(conv9)

    ########## CLASSIFICATION SIDE ##########

    class_inputs = Input((None, None, num_channels), name='class_inputs')
    _, _, _, _, drop5 = wire_shared_weights(shared_weights=shared_weights,
                                            prev_layer=class_inputs)

    conv5 = Conv2D(1024//ds, 3, activation='relu',
    ...
    # logits output
    class_outputs = Dense(2, name='class_outputs')(x)
